    ...
    process(_v, _w, parameters) {
        ...
        for (let c = 0; c < _x.length; ++c) {
            ...
            for (let _z = 0; _z < _y.length; ++_z)
                _w[parameters.bypass[_z] ?? parameters.bypass[0]][c][_z] = _y[_z];
        }
        return this._s;
    }
}
...
class _P extends AudioWorkletProcessor {
    ...
    constructor(_H) {
        ...
        for (let c = 0; c < _I; ++c)
            this.buffer[c] = new Float32Array(_R);
    }
    process(_v, _w, parameters) {
        const _x = _v[0];
        const _D = _w[0];
    ...
    _U(_W, _X) {
        ...
        let __ = (_Z - 1);
        while (_Z < 0)
            _Z += this.buffer[_W].length;
        while (__ < 0)
            __ += this.buffer[_W].length;
        const frac = _Y - ~~_Y;
...
registerProcessor("gain-processor", _31);
class _41 extends AudioWorkletProcessor {
    ...
    process(_v, _w, parameters) {
        ...
        if (_h1)
            this._i1(cutoff[0], q[0]);
        for (let c = 0; c < _x.length; ++c) {
            const _y = _x[c];
            const _E = _D[c];
            for (let _z = 0; _z < _y.length; ++_z) {
                if (!_h1)
                    this._i1(cutoff[_z] ?? cutoff[0], q[_z] ?? q[0]);
...
class _B1 extends AudioWorkletProcessor {
    ...
    static _G1 = 0.28;
    static _H1 = 0.7;
    static _I1 = [1116, 1188, 1277, 1356, 1422, 1491, 1557, 1617];
    ...
    constructor(_H) {
        ...
        for (let c = 0; c < _I; ++c) {
            ...
            for (let i = 0; i < _B1._C1; ++i)
                this._O1[c][i] = new _q1(_Q1[c % _Q1.length][i]);
            for (let i = 0; i < _B1._D1; ++i)
                this._P1[c][i] = new _A1(_R1[c % _R1.length][i]);
        }
        this._S1(0.5);
        this._y1(0.5);
        for (let c = 0; c < _I; ++c)
            for (let i = 0; i < _B1._D1; ++i)
                this._P1[c][i]._w1(0.5);
    }
    process(_v, _w, parameters) {
        const _x = _v[0];
        const _D = _w[0];
        ...
        for (let c = 0; c < _x.length; ++c) {
            ...
            for (let _z = 0; _z < _y.length; ++_z) {
                ...
                for (let i = 0; i < _B1._C1; ++i)
                    out += this._O1[c][i].process(_L);
                for (let i = 0; i < _B1._D1; ++i)
                    out = this._P1[c][i].process(out);
                if (bypass[_z] ?? bypass[0])
                    continue;
                const _O = (mix[_z] ?? mix[0]);
                _E[_z] *= (1 - _O);
